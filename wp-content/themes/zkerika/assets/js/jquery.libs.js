...
;(function ($, window, document, undefined) {
    ...
    Animate.prototype.swap = function () {
        ...
        var items_leave = [],
            items_incoming = [],
            paginate_item = this.core.settings.items,
            in_leave_thread = false,
            in_incoming_thread = false,
            all_items = this.core.$stage.children();
        if (this.core.current() > this.previous) {
            all_items.each(function (index) {
                if (this === previous[0]) {
                    in_leave_thread = true;
                }
                if (this === next[0]) {
                    in_leave_thread = false;
                    in_incoming_thread = true;
                }
                if (in_leave_thread) {
                    items_leave.push(this);
                }
                if (in_incoming_thread) {
                    items_incoming.push(this);
                }
                if (items_incoming.length >= paginate_item)
                    return false;
            });
            if (items_incoming.length > items_leave.length) {
                var diff = items_incoming.length - items_leave.length;
                items_incoming.splice(0, diff);
            }
        }
        else {
            var old_items_in_viewport = [],
                new_items_inviewport = [],
                items_stand_inviewport = [];
            all_items.each(function (index) {
                var end_viewport = Math.min(all_items.length - 1, index + paginate_item - 1);
                if (this === previous[0]) {
                    for (var i = index; i <= end_viewport; i++) {
                        if ($.inArray(i, new_items_inviewport) > -1)
                            items_stand_inviewport.push(i);
                        old_items_in_viewport.push(i);
                    }
                }
                if (this === next[0]) {
                    for (var i = index; i <= end_viewport; i++) {
                        new_items_inviewport.push(i);
                    }
                }
            });
            old_items_in_viewport.forEach(function (item) {
                if ($.inArray(item, items_stand_inviewport) === -1)
                    items_leave.push(all_items[item]);
            });
            new_items_inviewport.forEach(function (item) {
                if ($.inArray(item, items_stand_inviewport) === -1)
                    items_incoming.push(all_items[item]);
            });
        }
        //
        if (outgoing) {
            items_leave.forEach(function (item, index) {
                if (index >= items_incoming.length)
                    return false;
                $(item).one($.support.animation.end, clear)
                    .css({'left': left + 'px'})
        ...
        if (incoming) {
            items_incoming.forEach(function (item) {
                $(item).one($.support.animation.end, clear)
    ...
    $(window).on('load',function () {
        ...
        $.fn.owlFilter = function (data, callback) {
            var owl = this,
                ...
                $elemCopy = $('<div></div>').css('display', 'none');
            ...
            owl.owlRemoveItem(0);
            owl_data.options.dotsData = false;
        ...
        $(".cms-carousel").each(function () {
            ...
            /* Filter */
            if ($this.parents('.cms-carousel-wraper').find('.cms-carousel-filter').length > 0) {


                $this.parents('.cms-carousel-wraper').find('.cms-carousel-filter').on('click', '.filter', function (slider_settings) {
                    var filter_data = $(this).data('filter');
                    /* return if current */
                    if ($(this).hasClass('active')) return;
                    /* active current */
                    $(this).parents('.cms-carousel-filter').find('.filter').removeClass('active');
                    $(this).addClass('active');
                    /* Filter */
                    $this.owlFilter(filter_data, function (_owl) {
                        $(_owl).find('.item').each(function (even) {
                            $(this)
                                .addClass('__loading')
        ...
        /* mousewheel */
        var owl = $('.cms-carousel-mousewheel');
...
!function (t) {
    ...
    e.defaults = {
        ...
        var n = this.$element, a = n.parent(), s = this.$back_text, i = this.$front_text, r = this.options, o = n.attr("data-valuetransitiongoal"), h = n.attr("data-valuemin") || 0, d = n.attr("data-valuemax") || 100, f = a.hasClass("vertical"), u = r.update && "function" == typeof r.update ? r.update : e.defaults.update, c = r.done && "function" == typeof r.done ? r.done : e.defaults.done, p = r.fail && "function" == typeof r.fail ? r.fail : e.defaults.fail;
        ...
        if ("center" === r.display_text && !s && !i) {
            ...
            var g;
            f ? (g = a.css("height"), s.css({height: g, "line-height": g}), i.css({
                height: g,
                "line-height": g
...
jQuery(function (t) {
    "use strict";
    t(".cms-counter-wraper .counter-number").each(function () {
        var a = {
            useEasing: !1,
            useGrouping: !1,
            separator: ",",
            decimal: "."
        }, i = t(this).attr("data-digit"), r = i.split(".");
        void 0 === r[1] && (r[1] = 0);
        var e = t(this).attr("data-prefix"), n = t(this).attr("data-suffix");
        void 0 != e && (a.prefix = e), void 0 != n && (a.suffix = n);
        s = 0;
        if ("random" == t(this).attr("data-type"))var s = Math.floor(Math.random() * i * 2);
        t(this).waypoint(function () {
            new countUp(t(this).attr("id"), s, i, r[1].length, 0, a).start()
