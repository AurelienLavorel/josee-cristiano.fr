...
class NectarLove {
	...
	function love_post($post_id, $action = 'get')
	{
		...
		$unit = esc_html__('likes','zk-erika');
		switch($action) {
	...
	function add_love($unit = '', $show_icon = false) {
		global $post;
		...
		$output = $this->love_post($post->ID);
  		...
  		$class = 'nectar-love';
  		$icon = 'fa fa-heart-o';
  		$title = esc_html__('Like this', 'zk-erika');
  		if(empty($unit)) $unit = esc_html__('Likes','zk-erika');
		if( isset($_COOKIE['nectar_love_'. $post->ID]) ){
			$class = 'nectar-love loved unselected';
			$icon = 'fa fa-heart';
			$title = esc_html__('You already liked this!', 'zk-erika');
		}
		$heart_icon = '';
		if($show_icon){
...
function post_favorite($return = '', $unit = '', $show_icon = false) {
	global $post_favorite;
	if($return == 'return') {
