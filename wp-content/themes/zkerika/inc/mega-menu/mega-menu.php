...
add_action( 'wp_nav_menu_item_custom_fields', 'zkerika_add_menu_submenu_type_fields', 10, 4 );

function zkerika_add_menu_submenu_type_fields( $item_id, $item, $depth, $args ) {
    if ( $depth )
        return;

    $title = esc_html__('Submenu Type', 'zk-erika'); 
    ...
    ?><p class="description description-wide description_width_100">
        ...
        <label for="edit-<?php echo ''.$key . '-' . $item_id; ?>">
            <select id="edit-<?php echo ''.$key . '-' . $item_id; ?>" class=" <?php echo ''.$key; ?>" name="<?php echo ''.$key . "[" . $item_id . "]"; ?>">
                <option value="standard" <?php echo ( $value == 'standard' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Standard Dropdown', 'zk-erika' ); ?></option>
                <option value="columns2" <?php echo ( $value == 'columns2' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( '2 columns dropdown', 'zk-erika' ); ?></option>
                <option value="columns3" <?php echo ( $value == 'columns3' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( '3 columns dropdown', 'zk-erika' ); ?>
                </option>
                <option value="columns4" <?php echo ( $value == 'columns4' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( '4 columns dropdown', 'zk-erika' ); ?></option>
                <option value="columns5" <?php echo ( $value == 'columns5' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( '5 columns dropdown', 'zk-erika' ); ?></option>
            </select>
        </label>
    </p>
    <?php
    $title = esc_html__('Sub menu column width ( Example: 200)', 'zk-erika');
    ...
    <?php
    $title = esc_html__('Side of Dropdown Elements', 'zk-erika');
    ...
    <p class="description description-wide description_width_100">
        ...
        <label for="edit-<?php echo ''.$key . '-' . $item_id; ?>">
            <select id="edit-<?php echo ''.$key . '-' . $item_id; ?>" class=" <?php echo ''.$key; ?>" name="<?php echo ''.$key . "[" . $item_id . "]"; ?>">
                <option value="autodrop_submenu" <?php echo ( $value == 'autodrop_submenu' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Auto drop', 'zk-erika' ); ?></option>
                <option value="drop_to_left" <?php echo ( $value == 'drop_to_left' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Drop To Left Side', 'zk-erika' ); ?></option>
                <option value="drop_to_right" <?php echo ( $value == 'drop_to_right' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Drop To Right Side', 'zk-erika' ); ?></option>
                <option value="drop_to_center" <?php echo ( $value == 'drop_to_center' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Drop To Center', 'zk-erika' ); ?></option>
                <option value="drop_full_width" <?php echo ( $value == 'drop_full_width' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Full width', 'zk-erika' ); ?></option>
            </select>
        </label>
    </p><?php
}
...
// widget_area.
add_action( 'wp_nav_menu_item_custom_fields', 'zkerika_add_menu_widget_area_fields', 10, 4 );

function zkerika_add_menu_widget_area_fields( $item_id, $item, $depth, $args ) {

    if(!$depth)
        return;

    $title    = esc_html__('Widget Area', 'zk-erika'); 
    ...
    ?><p class="description description-wide description_width_100 el_widget_area">
        ...
        <label for="edit-<?php echo ''.$key . '-' . $item_id; ?>">
            <select id="edit-<?php echo ''.$key . '-' . $item_id; ?>" class=" <?php echo ''.$key; ?>" name="<?php echo ''.$key . "[" . $item_id . "]"; ?>">
                <option value="" <?php echo ( $value == '' ) ? ' selected="selected" ' : ''; ?>><?php esc_html_e( 'Select Widget Area', 'zk-erika' ); ?></option>
...
// item_group.
add_action( 'wp_nav_menu_item_custom_fields', 'zkerika_add_menu_item_group_fields', 10, 4 );

function zkerika_add_menu_item_group_fields( $item_id, $item, $depth, $args ) {
    $title = esc_html__('Group', 'zk-erika'); 
    $key = "menu-item-group";
    ...
    <div>
        <label><?php esc_html_e('No', 'zk-erika'); ?>
        ...
        <label><?php esc_html_e('Yes', 'zk-erika'); ?>
...
/* one page */
add_action( 'wp_nav_menu_item_custom_fields', 'zkerika_add_menu_is_onepage_fields', 10, 4 );

function zkerika_add_menu_is_onepage_fields($item_id, $item, $depth, $args){

    $title = esc_html__('Is OnePage', 'zk-erika'); 
...
// hide_link.
add_action( 'wp_nav_menu_item_custom_fields', 'zkerika_add_menu_hide_link_fields', 10, 4 );

function zkerika_add_menu_hide_link_fields( $item_id, $item, $depth, $args ) {
    if(!$depth)
        return;

    $title = esc_html__('Hide title', 'zk-erika'); 
...
// menu icon.
add_action( 'wp_nav_menu_item_custom_fields', 'zkerika_add_menu_icon_fields', 10, 4 );

function zkerika_add_menu_icon_fields( $item_id, $item, $depth, $args ) {

    $title = esc_html__('Menu Icon', 'zk-erika'); $key = "menu-item-menu_icon"; $value = $item->menu_icon;

    ?>
    <div id="<?php echo ''.$key . '-' . $item_id . '-popup'; ?>" data-item_id="<?php echo ''.$item_id;?>" class="menu_icon_wrap" style="display:none;">
        <?php
        $icons = apply_filters('theme/menu/icons', zkerika_font_awesome());
    ...
    <p class="description description-wide obtheme_checkbox obtheme_mega_menu obtheme_mega_menu_d1">
        <label for="edit-<?php echo ''.$key . '-' . $item_id; ?>">
            <?php echo esc_html( $title ); ?><br />
            <input type="text" value="<?php echo ''.$value; ?>" id="edit-<?php echo ''.$key . '-' . $item_id; ?>" class=" <?php echo ''.$key; ?>" name="<?php echo ''.$key . "[" . $item_id . "]"; ?>" />
            <input alt="#TB_inline?height=auto&width=auto&inlineId=<?php echo ''.$key . '-' . $item_id . '-popup'; ?>" title="<?php esc_html_e( 'Click to browse icon', 'zk-erika' ) ?>" class="thickbox button-secondary submit-add-to-menu" type="button" value="<?php esc_html_e( 'Browse Icon', 'zk-erika' ) ?>" />
...
// menu background.
add_action( 'wp_nav_menu_item_custom_fields', 'zkerika_add_menu_image_background_fields', 10, 4 );

function zkerika_add_menu_image_background_fields( $item_id, $item, $depth, $args ) {

    if($depth)
        return;

    $title = esc_html__('DropDown Background Image', 'zk-erika'); $key = "menu-item-bg_image"; $value = $item->bg_image;
    ...
    <p class="description description-wide obtheme_checkbox obtheme_mega_menu obtheme_mega_menu_d2">
        <label for="edit-<?php echo ''.$key . '-' . $item_id; ?>">
            <span class='obtheme_long_desc'><?php echo esc_html( $title ); ?></span><br />
            <input type="text" value="<?php echo ''.$value; ?>" id="edit-<?php echo ''.$key . '-' . $item_id; ?>" class=" <?php echo ''.$key; ?>" name="<?php echo ''.$key . "[" . $item_id . "]"; ?>" />
            <button id="browse-edit-<?php echo ''.$key . '-' . $item_id; ?>" class="set_custom_images button button-secondary submit-add-to-menu"><?php esc_html_e( 'Browse Image', 'zk-erika' ); ?></button>
            <a class="button btn_clear button-primary" href="javascript: void(0);">Clear</a>
        </label>
    </p>
    ...
    <?php $title = esc_html__('Background Color', 'zk-erika'); $key = "menu-item-bg_color"; $value = $item->bg_color; ?>
    ...
    <?php $title = esc_html__('Icon Color', 'zk-erika'); $key = "menu-item-icon_color"; $value = $item->icon_color; ?>
    ...
    <?php $title = esc_html__('Icon Font Size', 'zk-erika'); $key = "menu-item-icon_font_size"; $value = $item->icon_font_size; ?>
...
// menu background.
add_action( 'wp_nav_menu_item_custom_fields', 'zkerika_add_menu_el_class_fields', 10, 4 );

function zkerika_add_menu_el_class_fields( $item_id, $item, $depth, $args ) {

    $title = esc_html__('Extra Class', 'zk-erika'); $key = "menu-item-el_class"; $value = $item->el_class;
...
class Walker_Nav_Menu_Edit_Custom extends Walker_Nav_Menu  {
    ...
    function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        if ( ! empty( $item->_invalid ) ) {
            ...
            $title = sprintf( esc_html__( '%s (Invalid)', 'zk-erika'), $item->title );
        } elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
            $classes[] = 'pending';
            /* translators: %s: title of menu item in draft status */
            $title = sprintf( esc_html__('%s (Pending)', 'zk-erika'), $item->title );
        }
    ...
    <li id="menu-item-<?php echo esc_attr($item_id); ?>" class="<?php echo implode(' ', $classes ); ?>">
        <dl class="menu-item-bar">
            <dt class="menu-item-handle">
                <span class="item-title"><span class="menu-item-title"><?php echo esc_html( $title ); ?></span> <span class="is-submenu" <?php echo esc_html($submenu_text); ?>><?php esc_html_e( 'sub item' , 'zk-erika'); ?></span></span>
                <span class="item-controls">
                    ...
                    <span class="item-order hide-if-js">
                        ...
                        ?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up', 'zk-erika'); ?>">&#8593;</abbr></a>
                        |
                        <a href="<?php
                        echo esc_url( wp_nonce_url(
                            add_query_arg(
                        ...
                        ?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down', 'zk-erika'); ?>">&#8595;</abbr></a>
                    </span>
                    <a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>" title="<?php esc_attr_e('Edit Menu Item', 'zk-erika'); ?>" href="<?php
                    ...
                    ?>"><?php esc_html_e( 'Edit Menu Item', 'zk-erika' ); ?></a>
                </span>
            </dt>
        </dl>
        ...
        <div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
            <?php if( 'custom' == $item->type ) : ?>
                <p class="field-url description description-wide">
                    <label for="edit-menu-item-url-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e( 'URL', 'zk-erika' ); ?><br />
            ...
            <p class="description description-thin">
                <label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'Navigation Label', 'zk-erika' ); ?><br />
                    <input type="text" id="edit-menu-item-title-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
                </label>
            </p>
            <p class="description description-thin">
                <label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'Title Attribute', 'zk-erika' ); ?><br />
            ...
            <p class="field-link-target description">
                <label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
                    ...
                    <?php esc_html_e( 'Open link in a new window/tab', 'zk-erika' ); ?>
                </label>
            </p>
            <p class="field-css-classes description description-thin">
                <label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'CSS Classes (optional)', 'zk-erika' ); ?><br />
                    <input type="text" id="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
                </label>
            </p>
            <p class="field-xfn description description-thin">
                <label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'Link Relationship (XFN)', 'zk-erika' ); ?><br />
                    <input type="text" id="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
                </label>
            </p>
            <p class="field-description description description-wide">
                <label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e( 'Description', 'zk-erika' ); ?><br />
                    <textarea id="edit-menu-item-description-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr($item_id); ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
                    <span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.', 'zk-erika' ); ?></span>
                </label>
            </p>
            ...
            <p class="field-move hide-if-no-js description description-wide">
                <label>
                    <span><?php esc_html_e( 'Move', 'zk-erika' ); ?></span>
                    <a href="#" class="menus-move menus-move-up" data-dir="up"><?php esc_html_e( 'Up one', 'zk-erika' ); ?></a>
                    <a href="#" class="menus-move menus-move-down" data-dir="down"><?php esc_html_e( 'Down one', 'zk-erika' ); ?></a>
                    <a href="#" class="menus-move menus-move-left" data-dir="left"></a>
                    <a href="#" class="menus-move menus-move-right" data-dir="right"></a>
                    <a href="#" class="menus-move menus-move-top" data-dir="top"><?php esc_html_e( 'To the top', 'zk-erika' ); ?></a>
                </label>
            </p>
            ...
            <div class="menu-item-actions description-wide submitbox">
                <?php if( 'custom' != $item->type && $original_title !== false ) : ?>
                    <p class="link-to-original">
                        <?php printf( esc_html__('Original: %s', 'zk-erika' ), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
                    </p>
                <?php endif; ?>
                ...
                ) ); ?>"><?php esc_html_e( 'Remove', 'zk-erika' ); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a class="item-cancel submitcancel hide-if-no-js" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url( add_query_arg( array( 'edit-menu-item' => $item_id, 'cancel' => time() ), admin_url( 'nav-menus.php' ) ) );
                ?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel', 'zk-erika' ); ?></a>
            </div>
